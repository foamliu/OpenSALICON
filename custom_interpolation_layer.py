...
class custom_interpolation_layer(caffe.Layer):
    ...
    def forward(self, bottom, top):
        ...
        interpolated_data = scipy.ndimage.interpolation.zoom(bottom[0].data, self.SCALE_FACTOR, np.dtype(np.float32),
                                                             mode='nearest', order=1)
        top[0].data[...] = interpolated_data

    def backward(self, top, propagate_down, bottom):
        # interpolate top diff to bottom diff
        interpolated_data = scipy.ndimage.interpolation.zoom(top[0].diff, self.REVERSE_SCALE_FACTOR,
                                                             np.dtype(np.float32), mode='nearest', order=1)
